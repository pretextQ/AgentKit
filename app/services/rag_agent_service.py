...
class RagAgentService:
    ...
    async def _initialize_agent(self):
        ...
        self.agent = create_react_agent(
            ...
            pre_model_hook=trim_messages_middleware,
        )
        ...
        try:
            ...
            async for token, metadata in self.agent.astream(
                ...
                if message_type in ("AIMessage", "AIMessageChunk"):
                    ...
                    if not has_content_from_blocks:
                        direct_content = getattr(token, 'content', None)
    ...
    def get_session_history(self, session_id: str) -> list:
        ...
        try:
            ...
            elif isinstance(checkpoint_tuple, (list, tuple)) and len(checkpoint_tuple) > 0:
                checkpoint_data = checkpoint_tuple[0] if checkpoint_tuple[0] else {}
            ...
            if not checkpoint_data or not isinstance(checkpoint_data, dict):
                logger.info(f"获取会话历史: {session_id}, 无法解析检查点数据")
                return []
        ...
        try:
            if hasattr(self.checkpointer, 'storage'):
                if session_id in self.checkpointer.storage:
                    ...
                    logger.info(f"会话不存在，无需清空: {session_id}")
                    return False

            logger.warning(f"无法清除会话历史: {session_id}, checkpointer 不支持直接删除")
            return False
